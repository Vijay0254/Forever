...
const ShopContextProvider = ({ children }) => {
  ...
  async function fetchProducts(){
    try{
      const response = await axios.get(`/api/product/get/all`, {withCredentials: true, headers: 'https://forever-frontend-sandy.vercel.app'})
  ...
  async function getUserCart(){
    try{
      const response = await axios.get(`/api/cart/get`, {withCredentials: true, headers: 'https://forever-frontend-sandy.vercel.app'})
      if(response.data.message == "Internal Server Error"){
        navigate('/500')
      }
      if(response.data.success){
  ...
  async function verifyToken(){
    try{
      const response = await axios.get(`/api/auth/user/verify`, {withCredentials: true, headers: 'https://forever-frontend-sandy.vercel.app'})
      if(response.data.message == "Internal Server Error"){
        navigate('/500')
      }
      if(response.data.success){
  ...
  async function addToCart(itemId, size){
    try{
      ...
      if(user){
        try{
          const response = await axios.post(`/api/cart/add`, {itemId: itemId, size: size}, {withCredentials: true, headers: 'https://forever-frontend-sandy.vercel.app'})
          if(response.data.message == "Internal Server Error"){
            navigate('/500')
          }
  ...
  async function updateQuantity(itemId, size, quantity){
    try{
      ...
      if(user){
        try{
          const response = await axios.post(`/api/cart/update`, {itemId: itemId, size: size, quantity: quantity}, {withCredentials: true, headers: 'https://forever-frontend-sandy.vercel.app'})
          if(response.data.message == "Internal Server Error"){
            navigate('/500')
          }
